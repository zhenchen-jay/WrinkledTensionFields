    ...
    bool isLocalProj)
{
    ...
    MatrixXd bderiv;
    bderiv.resize(4, 18 + 3 * nedgedofs);
    std::vector<MatrixXd > bhess;
    ...
    MatrixXd aderiv;
    aderiv.resize(4, 18 + 3 * nedgedofs);
    ...
    std::vector<MatrixXd > ahess = bhess;
    if (hessian)
    {
        for (int i = 0; i < 4; i++)
        {
    ...
    if (derivative)
    {
        ...
        *derivative += term3 * m4(1, 1) * aderiv.row(3);
        ...
        double term4 = lameBeta * 2.0 / pow(deta, 2) * (aadj*b*abaradj*bbar).trace() / detabar;
        ...
        *derivative += term4 * aadj(1, 1) * aderiv.row(3);
        ...
        double term5 = lameAlpha * (abaradj*bbar / detabar - aadj * b / deta).trace() * -1.0 / deta;
        ...
        *derivative += term5 * aadj(1, 1) * bderiv.row(3);
        ...
        double term6 = lameAlpha * (abaradj*bbar / detabar - aadj * b / deta).trace() * 1.0 / pow(deta, 2) * (aadj*b).trace();
    ...
    {
        hessian->setZero();
        MatrixXd aadjda = aadj(0, 0) * aderiv.row(0);
        ...
        *hessian += term1 * (m3(0, 0) * aderiv.row(0).transpose() + m3(0, 1) * aderiv.row(2).transpose()) * bderiv.row(3);
        ...
        MatrixXd m4db = m4(0, 0) * bderiv.row(0);
        ...
        *hessian += term3 * (m7(0, 0) * bderiv.row(0).transpose() + m7(0, 1) * bderiv.row(2).transpose()) * aderiv.row(3);
        ...
        MatrixXd m8da = m8(0, 0) * aderiv.row(0);
        ...
        *hessian += term5 * aderiv.row(0).transpose() * aderiv.row(3);
        ...
        MatrixXd m9db = m9(0, 0) * bderiv.row(0);
        ...
        *hessian += term6 * m9db.transpose() * aadjda;
        ...
        MatrixXd m10da = m10(0, 0) * aderiv.row(0);
        ...
        *hessian += term8 * m12(1, 1) * bhess[3];
        ...
        MatrixXd m13db = m13(0, 0) * bderiv.row(0);
        ...
        *hessian += term10 * m15(1, 1) * ahess[3];
        ...
        MatrixXd m16da = m16(0, 0) * aderiv.row(0);
        ...
        *hessian += term11 * m13db.transpose() * aadjda;
        ...
        double term12 = lameBeta * 2.0 / pow(deta, 2) * (aadj*b*abaradj*bbar).trace() / detabar;
        ...
        *hessian += term12 * aadj(1, 1) * ahess[3];
        ...
        double term13 = lameBeta * -4.0 / pow(deta, 2) / deta * (aadj*b*abaradj*bbar).trace() / detabar;
        *hessian += term13 * aadjda.transpose() * aadjda;

        // end term 1

        double term14 = lameAlpha * -1.0 * (abaradj*bbar / detabar - aadj * b / deta).trace() / deta;
        ...
        *hessian += term14 * aadj(1, 1) * bhess[3];
        ...
        double term15 = lameAlpha * 1.0 * (abaradj*bbar / detabar - aadj * b / deta).trace() / pow(deta, 2);
        MatrixXd badjda = badj(0, 0) * aderiv.row(0);
        ...
        *hessian += term15 * aadjda.transpose() * badjda;
        MatrixXd aadjdb = aadj(0, 0) * bderiv.row(0);
        ...
        *hessian += term15 * aadjda.transpose() * aadjdb;
        ...
        double term16 = lameAlpha * 1.0 * (abaradj*bbar / detabar - aadj * b / deta).trace() / pow(deta, 2) * (aadj*b).trace();
        ...
        *hessian += term16 * aderiv.row(0).transpose() * aderiv.row(3);
        ...
        double term17 = lameAlpha * 1.0 * (abaradj*bbar / detabar - aadj * b / deta).trace() / pow(deta, 2);
        *hessian += term17 * aadjdb.transpose() * aadjda;
        *hessian += term17 * badjda.transpose() * aadjda;

        double term18 = lameAlpha * -2.0 * (abaradj*bbar / detabar - aadj * b / deta).trace() / pow(deta, 3) * (aadj*b).trace();
