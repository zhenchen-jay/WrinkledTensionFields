...
void WTFModel::initialization(const WTFSetup setup, const WTFState state, std::map<int, double>* clampedDOFs, std::string filePath, bool isUsePosHess, bool isParallel)
{
	_setup = setup;
...
void WTFModel::setProjM(std::map<int, double>* clampedDOFs)
{
	_freeAmp = 0;
	_freePhi = 0;
	_freeDphi = 0;
...
Eigen::SparseMatrix<double> WTFModel::buildIntegrabilityConstraints()
{
	int nfaces = _state.baseMesh.nFaces();
	int nedges = _state.baseMesh.nEdges();
	int nverts = _state.basePos.rows();
...
void WTFModel::convertCurState2Variables(const WTFState curState, TVector& x)
{
...
void WTFModel::convertVariables2CurState(TVector x, WTFState& curState)
{
...
double WTFModel::value(const TVector& x)
{
	convertVariables2CurState(x, _state);
	std::shared_ptr<WTFShell> reducedShell;
	double energy = 0;
	reducedShell = std::make_shared<WTFShell>(_setup, _state, _isUsePosHess, _isParallel);
	energy = reducedShell->elasticReducedEnergy(NULL, NULL);
	return energy;

}

double WTFModel::stretchingValue(const TVector& x)
{
	convertVariables2CurState(x, _state);
...
double WTFModel::bendingValue(const TVector& x)
{
	convertVariables2CurState(x, _state);
...
void WTFModel::gradient(const TVector& x, TVector& grad)
{
...
void WTFModel::hessian(const TVector& x, THessian& hessian)
{
	std::shared_ptr<WTFShell> reducedShell;
	double energy = 0;
	convertVariables2CurState(x, _state);
...
void WTFModel::save(int curIterations, TimeCost curTimeCost, double stepSize, double oldEnergy, double curEnergy, double fDelta, double ampDelta, double dphiDelta, double reg, double* projGradNorm, bool PSDHess, bool isSaveAllIntermediate)
{
...
void WTFModel::getUpsampledWrinkledMesh(Eigen::MatrixXd& NV, Eigen::MatrixXi& NF, Eigen::MatrixXd &upsampledTFTV, Eigen::MatrixXi &upsampledTFTF, Eigen::MatrixXd &soupPhiV, Eigen::MatrixXi &soupPhiF, Eigen::MatrixXd &soupProblemV, Eigen::MatrixXi &soupProblemF, Eigen::VectorXd &upsampledAmp, Eigen::VectorXd &soupPhi)
{
...
Eigen::VectorXd WTFModel::getProjectedGradient(const TVector &x)
{
	Eigen::VectorXd g;
...
void WTFModel::testValueAndGradient(const TVector &x)
{
...
void WTFModel::testGradientAndHessian(const TVector& x)
{
...
void WTFModel::checkPD4ConstraintedHess(const TVector& x)
{
	bool isUsePosHess = _isUsePosHess;
	_isUsePosHess = false;
