...
class MoreThuente {
 ...
 public:
  ...
  static Scalar linesearch(const TVector &x, const TVector &searchDir, ProblemType &objFunc, const  Scalar alpha_init = 1.0, Scalar max_step = 1e15) {
    // assume step width
    ...
    cvsrch(objFunc, xx, fval, g, ak, s, max_step);

    return ak;
  }

  static int cvsrch(ProblemType &objFunc, TVector &x, Scalar f, TVector &g, Scalar &stp, TVector &s, Scalar max_step = 1e15) {
