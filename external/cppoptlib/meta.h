...
public:
    ...
    void reset() {
        iterations = 0;
        xDelta = std::numeric_limits<double>::infinity();
        fDelta = std::numeric_limits<double>::infinity();
        gradNorm = std::numeric_limits<double>::infinity();
        condition = 0;
    }
...
Status checkConvergence(const Criteria<T> &stop, const Criteria<T> &current) {
    ...
    if ((stop.iterations > 0) && (current.iterations > stop.iterations)) {
        std::cout<<"Iteration limit reached: "<<stop.iterations<<std::endl;
        return Status::IterationLimit;
    }
    if ((stop.xDelta > 0) && (current.xDelta < stop.xDelta)) {
        std::cout<<"Change in parameter vector too small: "<<current.xDelta<<std::endl;
        return Status::XDeltaTolerance;
    }
    if ((stop.fDelta > 0) && (current.fDelta < stop.fDelta)) {
        std::cout<<"Change in cost function value too small: "<<current.fDelta<<std::endl;
        return Status::FDeltaTolerance;
    }
    if ((stop.gradNorm > 0) && (current.gradNorm < stop.gradNorm)) {
        std::cout<<"Gradient vector norm too small: "<<current.gradNorm<<std::endl;
        return Status::GradNormTolerance;
    }
    if ((stop.condition > 0) && (current.condition > stop.condition)) {
        std::cout<<"Condition of Hessian/Covariance matrix too large: "<<current.condition<<std::endl;
