...
class Problem {
 public:
  ...
  using THessian  = Eigen::SparseMatrix<Scalar>;
...
#pragma GCC diagnostic pop
  ...
  virtual bool checkHessian(const TVector &x, int accuracy = 3) {
    ...
    THessian actual_hessian;
    actual_hessian.resize(D, D);
    actual_hessian.setZero();

    THessian expected_hessian;
    expected_hessian.resize(D, D);
    hessian(x, actual_hessian);
    finiteHessian(x, expected_hessian, accuracy);

    for (TIndex d = 0; d < D; ++d) {
      for (TIndex e = 0; e < D; ++e) {
        Scalar scale = std::max(static_cast<Scalar>(std::max(fabs(actual_hessian.coeffRef(d, e)), fabs(expected_hessian.coeffRef(d, e)))), Scalar(1.));
        if(fabs(actual_hessian.coeffRef(d, e)- expected_hessian.coeffRef(d, e))>1e-1 * scale)
          return false;
      }
    }
    return true;
    ...
    if(accuracy == 0) {
      for (TIndex i = 0; i < x.rows(); i++) {
        for (TIndex j = 0; j < x.rows(); j++) {
          ...
          xx[i] -= eps;
          Scalar f3 = value(xx);
          hessian.coeffRef(i, j) = (f1 - f2 - f3 + f4) / (eps * eps);
      ...
      */
      for (TIndex i = 0; i < x.rows(); i++) {
        for (TIndex j = 0; j < x.rows(); j++) {
          ...
          hessian.coeffRef(i, j) = (-63 * term_1+63 * term_2+44 * term_3+74 * term_4)/(600.0 * eps * eps);
        }
      }
    }
