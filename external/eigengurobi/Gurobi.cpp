	...
	const VectorXd& X0)
{
	...
	if(X0.size() == nrvar_)
		model_.set(GRB_DoubleAttr_Start, vars_, X0.data(), nrvar_);
	...
	const VectorXd& X0)
{
	...
	inform();
	std::cout << "feasibility tolerance: " << feasibilityTolerance() << std::endl;
	std::cout << "optimality tolerance: " << optimalityTolerance() << std::endl;
