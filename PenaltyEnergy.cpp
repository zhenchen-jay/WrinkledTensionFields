...
#include "collisionDetectionWrapper/Distance.h"
#include "Collision.h"
...
//This energy is only C0 continuous
/*
...
{ 
    double energy = 0;
    std::vector<std::vector<double> > barys;
    std::vector<std::vector<double> > gridSDFs;
    std::vector<double> dis = obs.computeSDF(V, barys, gridSDFs);
    ...
    for (int i = 0; i < V.rows(); i++)
    {
        ...
        if (dis[i] < 0)
        {
            //std::cout << i << " dis : " << dis[i] << " barys : " << barys[i][0] << " " << barys[i][1] << " " << barys[i][2] << std::endl;
            
            energy += 0.5 * penaltyK * dis[i] * dis[i];
            if (dE)
            {
                Eigen::Vector3d partialdE;
                partialdE.setConstant(0);
                double coeff[3];
                double sign[3];
                for (int j = 0; j < 2 ; j++)
                {
                    j == 0 ? coeff[2] = 1 - barys[i][2] : coeff[2] = barys[i][2];
                    j == 0 ? sign[2] = -1 : sign[2] = 1;
                    for (int k = 0; k < 2 ; k++)
                    {
                        k == 0 ? coeff[1] = 1 - barys[i][1] : coeff[1] = barys[i][1];
                        k == 0 ? sign[1] = -1 : sign[1] = 1;
                        for (int l = 0; l < 2 ; l++)
                        {
                            l == 0 ? coeff[0] = 1 - barys[i][0] : coeff[0] = barys[i][0];
                            l == 0 ? sign[0] = -1 : sign[0] = 1;
                            int index = l + 2 * k + 4 * j;
                            //std::cout << "index : " << index << "gridSDF : " << gridSDFs[i][index] << std::endl;
                            for (int m = 0; m < 3; m++)
                            {
                                partialdE(m) = sign[m] * 1./ obs.gridSize[m] * coeff[(m+1)%3] * coeff[(m+2)%3] * gridSDFs[i][index];
                                if (hE)
                                {
                                    partialhE(m, (m+1)%3) += penaltyK * dis[i] * gridSDFs[i][index] * sign[m] * (1./obs.gridSize[m]) * sign[(m+1)%3] * (1./obs.gridSize[(m+1)%3]) * coeff[(m+2)%3];
                                    partialhE(m, (m+2)%3) += penaltyK * dis[i] * gridSDFs[i][index] * sign[m] * (1./obs.gridSize[m]) * sign[(m+2)%3] * (1./obs.gridSize[(m+2)%3]) * coeff[(m+1)%3];              }
                            }
                            dE->segment(3*i, 3) += partialdE;
                        }
                    }
                }
                if (hE)
                {
                    partialdE = dE->segment(3 * i , 3);
                    partialhE += penaltyK * partialdE * partialdE.transpose();
                    if(isProjHess) 
                        partialhE = lowRankApprox(partialhE);

                    for (int j = 0; j < 3; j++)
                    {
                        for (int k = 0; k < 3; k++)
                            hE->push_back(Eigen::Triplet<double>(3 * i + j, 3 * i + k, partialhE(j, k)));
                    }
                }
                dE->segment(3*i, 3) *= penaltyK * dis[i];
//                std::cout << i << " distance : " << dis[i] << std::endl ;
            }
        }
    }
    if (dE)
        ...
        if (it == setup.clampedDOFs.end())
        {
            ...
            for (int j = 0; j < setup.obs.size(); j++) {
                ...
                for (int k = 0; k < hits.size(); k++) 
                {
                    ...
                    // double q0bary, q1bary, q2bary;
                    // Eigen::Matrix3d dbarys;
                    // Distance::vertexFaceDistance(p0, q0, q1, q2, q0bary, q1bary, q2bary, &dbarys);
                    // Eigen::Vector3d normal = p0 - q0bary * q0 - q1bary * q1 - q2bary * q2;
                    // //Eigen::Vector3d normal = p0;
                    // Eigen::Matrix3d dq = q0 * dbarys.row(0) + q1 * dbarys.row(1) + q2 * dbarys.row(2);
                    // double dist = normal.norm();
                    // //std::cout << "dist : " << dist << std::endl;
                    // if (dist < setup.innerEta)
                    // {
                    //     energy += 0.5 * setup.penaltyK * pow(setup.innerEta - dist,2);                
                    //     if (dE)
                    //         dE->segment<3>(3 * i) += setup.penaltyK * (1. - setup.innerEta/ dist) * (normal.transpose() * (Eigen::MatrixXd::Identity(3,3) - dq)).transpose();
                    //     if (hE)
                    //     {
                    //         Eigen::Matrix3d localH = setup.penaltyK * ((1. - setup.innerEta/ dist) * Eigen::MatrixXd::Identity(3,3) + (setup.innerEta / (dist * dist * dist)) * normal * normal.transpose());
                    //         //localH = localH * (Eigen::MatrixXd::Identity(3,3) - dq);
                    //         localH -= setup.penaltyK * (1 - setup.innerEta/dist) * dq;
                    //         if(isProjHess) 
                    //             localH = lowRankApprox(localH);
                    //         for (int m = 0; m < 3; m++)
                    //         {
